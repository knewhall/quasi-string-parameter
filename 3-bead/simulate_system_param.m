...
function [pos_t, state_t, step, stopflag, switchtime_mat] = simulate_system_param(system, x, dt, nt, eps, stop)
    %rng(356);
    d = system.d;
    x = reshape(x, d, []);
    p = size(x,2);
    ...
    for step=1:nt
        ...
        for k=1:p
            ...
            for l=1:p
                ...
                v(:,k) = v(:,k) + a_ev * (x(:,k) - x(:,l))* exp(-norm(x(:,k)-x(:,l)).^2 / c_ev); %excluded volume force
            end
        end
        ...
        if ~isempty(stop)
            ...
            if planestop
                ...
                mindist = min(D);
               % if ((D(1) > 1 && D(2) > 1) || (D(1) > 1 && D(3) > 1) || (D(3) > 1 && D(2) > 1)) && min(D) < 0.1 %2bead cluster
                if D(1) > 1 && mindist < 0.1    
                    stopflag = 1;
                elseif D(1) < 0.4 && D(2) < 0.4 && D(3) < 0.4 %small triangle
                    stopflag = 2; 
                %elseif ((D(1) > 1 && D(2) < 0.4 && D(3) < 0.4) || (D(2) > 1 && D(1) < 0.4 && D(3) < 0.4) || (D(3) > 1 && D(2) < 0.4 && D(3) < 0.4))%coline 
                %    stopflag = 3;
                elseif (D(1) > 1 && D(2) > 1 && D(3) > 1) %unbound
                    stopflag = 4;
                end
            end
